...
- **Tech Stack:** Python 3.14+, Typer, Rich, Pydantic, Loguru, Pytest, Ruff, Pyright, MkDocs, MkDocs-Material
- **Key Patterns:** Modular architecture in `src/`; configuration in `src/repomgr/config/`; testing in `tests/`.
- **Relevant Folders:**
  - `src/repomgr/` – Main application code
  - `src/repomgr/config/` – Configuration management
...
- Ensure `.env` file is not present in the root, must be in `~/cred/repomgr/.env`.
- Secrets should not be committed.

## Related

- `src/repomgr/config/repomgr_paths.py`
